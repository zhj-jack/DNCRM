...
public class SynUserListener implements ServletContextListener {
    ...
    class GetUsersTask extends TimerTask {
        ...
        public void execute() {
    		...
    		try {
            	if(SynAuto){
            		PageData pd = getSynIds(dao,ssdDao);
     ...
     *获取需要同步的用户id,code 
     */
    public PageData getSynIds(DaoSupport dao,SqlServerDaoSupport ssdDao){
    	PageData rePd = new PageData();
    	try {
        	List<PageData> zlEmployeeList = (List<PageData>)ssdDao.findForList("SynUserMapper.findZlEmployee", "");
