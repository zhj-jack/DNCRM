							...
							<div class="panel panel-primary">
								...
								<div class="form-group form-inline">
									...
									<button class="btn btn-sm btn-info btn-sm" style="width: 9%; margin-left: 8px; height:31px"
									 title="提交" type="button" onclick="save();">提交</button> 
									<!-- <button class="btn btn-danger" style="width: 9%; margin-left:8px; height:31px" 
									 title="关闭" type="button" onclick="javascript:CloseSUWin('EditItem');">关闭</button> -->
								</div>
								<div class="form-group form-inline">
								...
								<div class="panel-body">
									<div class="form-group form-inline">
										...
										<select style="width: 22%" class="form-control" id="agreement_possible" name="agreement_possible">
										...
										<select style="width: 20%" class="form-control" id="market_type" name="market_type">
										...
										<select style="width: 20%" class="form-control" id="self_inferiority" name="self_inferiority">
															...
															<c:if test="${QX.cha == 1 }">
																<c:forEach items="${bjcList}" var="bjc" varStatus="vs">
																	<tr>
																		...
																		<td style="text-align: center;">${bjc.BJC_C}/${bjc.BJC_Z}</td>
																		<td style="text-align: center;">${bjc.BJC_SBJ}</td>
																		<td style="text-align: center;">${bjc.BJC_ZK}%</td>
																		<td style="text-align: center;">${bjc.BJC_ZHSBJ}</td>
								...
								<div class="panel-body">
								    ...
								    <div class="form-group form-inline">
										  ...
										  <input style="width:21%" type="text" id="SWXX_DJ" name="SWXX_DJ" value="${pd.SWXX_DJ}" class="form-control" placeholder="请输入定金比例">
										 
										  <label style="width:9%;margin-left:28px">排产款:</label> 
										  <input style="width:21%" type="text" id="SWXX_PCK" name="SWXX_PCK" value="${pd.SWXX_PCK}" class="form-control" placeholder="请输入排产款比例"> 
								    </div>
								    <div class="form-group form-inline">
								          <label style="width:9%;margin-left:10px">发货款:</label> 
										  <input style="width:21%" type="text" id="SWXX_FHK" name="SWXX_FHK" value="${pd.SWXX_FHK}" class="form-control" placeholder="请输入发货款比例">
										  
										  <label style="width:9%;margin-left:28px">货到工地款:</label> 
										  <input style="width:21%" type="text" id="SWXX_HDGDK" name="SWXX_HDGDK" value="${pd.SWXX_HDGDK}" class="form-control" placeholder="请输入货到工地款比例">
										 
										  <label style="width:9%;margin-left:28px">验收款:</label> 
										  <input style="width:21%" type="text" id="SWXX_YSK" name="SWXX_YSK" value="${pd.SWXX_YSK}" class="form-control" placeholder="请输入验收款比例"> 
								    </div>
								    
								    <label style="margin-left:10px">安装付款比例</label></br>
								    <div class="form-group form-inline">
								       <label style="width:9%;margin-left:10px">发货前:</label> 
									   <input style="width:21%" type="text" id="SWXX_FKBL_FHQ" name="SWXX_FKBL_FHQ" value="${pd.SWXX_FKBL_FHQ}" class="form-control" placeholder="请输入发货前比例">
									   
									   <label style="width:9%;margin-left:28px">货到工地:</label> 
									   <input style="width:21%" type="text" id="SWXX_FKBL_HDGD" name="SWXX_FKBL_HDGD" value="${pd.SWXX_FKBL_HDGD}" class="form-control" placeholder="请输入货到工地比例">
									   
									   <label style="width:9%;margin-left:28px">验收合格:</label> 
									   <input style="width:21%" type="text" id="SWXX_FKBL_YSHG" name="SWXX_FKBL_YSHG" value="${pd.SWXX_FKBL_YSHG}" class="form-control" placeholder="请输入验收合格比例">
								    </div>
								    ...
								    <label style="margin-left:10px">质保信息</label></br>
								     <div class="form-group form-inline">
								        ...
								        <input style="width:21%" type="text" id="SWXX_ZBJBL" name="SWXX_ZBJBL" value="${pd.SWXX_ZBJBL}" class="form-control" placeholder="请输入质保金比例">
...
<script type="text/javascript">
  ...
  function deleteRow(obj,elev_ids){
      ...
      if(typeof(bjc_id)!="undefined"&&bjc_id!=""){
      	$.post("<%=basePath%>e_offer/deleteBjc?BJC_ID="+bjc_id,function(result){
  ...
  function toEdit(obj,cod_id,models_id,bjc_id){
  	...
  	var url = "<%=basePath%>e_offer/offerView.do?COD_ID="+cod_id+"&rowIndex="+rowIndex+"&MODELS_ID="+models_id+"&BJC_ID="+bjc_id;
  ...
  {
  	var rowIndex = $(obj).parent().parent().index();
  	var url = "<%=basePath%>e_offer/offerView.do?OFFER_NUM="+offer_num+"&MODELS_ID="+modelsId+"&ELEV_IDS="+elev_ids+"&rowIndex="+rowIndex+"&ITEM_ID="+item_id;
	/*if(elevator_id=='1')
	{
...
//保存信息
	function save() 
	{
        ...
        var total=0;
        $("#tab1 tr:not(:first)").each(function()
		{
			var price =new Number($(this).find("td").eq(8).text());
			total=new Number(total)+price;
		});
        $("#total").val(total);
